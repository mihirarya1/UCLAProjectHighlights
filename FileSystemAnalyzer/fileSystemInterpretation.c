...
{
  int numBytes = (groupCount/8) + !!(groupCount%8);
  char buf [numBytes];
  if ( pread(fd, &buf, numBytes, computeOffset(bitmap)) == -1 )
    { fprintf(stderr,"Error in reading bitmap!\n"); exit(2); }

  for (int l=0; l<numBytes; l++)
    for (int m=0; m<8; m++)
	if ( (buf[l]&(1<<m))==0 && (l*8+m+1)<=groupBlockCount )
	  {
	    if ( bitmap==groupInfoData.bg_block_bitmap )
	      printf("BFREE,%d\n", l*8+m+1);
	    else
	      printf("IFREE,%d\n", l*8+m+1);
	  }
...
void groupInfo()
{
  if ( pread(fd, &groupInfoData, sizeof(groupInfoData), computeOffset(2)) == -1 )
    { fprintf(stderr,"Error reading group summary!\n"); exit(2); }

  groupBlockCount= sb.s_blocks_count;
  groupInodeCount= sb.s_inodes_count;
...
int main(int argc,  char *argv[] )
{
  ...
  superblockInfo();
  groupInfo();
  freeEntries(groupBlockCount, groupInfoData.bg_block_bitmap);
  freeEntries(groupInodeCount, groupInfoData.bg_inode_bitmap);
  inodeSummary();
